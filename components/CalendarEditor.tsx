  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-3">
        <h3 className="font-bold text-hibi-navy">
          {year}年{month}月
        </h3>
        <div className="flex gap-3 text-xs text-gray-500">
          <span>出勤 <strong className="text-blue-600">{workDays}</strong>日</span>
          <span>休み <strong className="text-gray-600">{offDays}</strong>日</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-7 gap-1">
        {cells.map((d, i) => {
          ...
          if (dayType === 'off') bg = 'bg-gray-100 text-gray-500 border-gray-200'
          if (dayType === 'holiday') bg = 'bg-red-100 text-red-600 border-red-200'
      ...
      <div className="mt-3 flex gap-4 justify-center text-xs text-gray-500">
        ...
        {!readOnly && (
          <span className="text-gray-400">※ タップで切替</span>
        )}
      </div>
    </div>
  )
