  ...
  const docs: { category: string; description?: string; items: DocItem[] }[] = [
    ...
    },
    {
      ...
      items: [
        ...
        { title: '給与計算マニュアル（奥寺さん用）', desc: '日比建設の月次給与計算。雇用形態4区分・日本人日給月給の有給手当（日給×有給日数）・フンさんの固定月給（396,105円）・ベトナム人の有給日給（20日枠超）・週所定での残業判定・端数処理（支給切上/控除切捨）・月締めの前提条件（月の完了＋職長チェック全完了）・支給額合計の使い方・自動検算・社労士提出用資料など', url: '/manual-payroll-okudera.html', icon: '💰', badge: 'NEW', updated: '2026-06-13' },
