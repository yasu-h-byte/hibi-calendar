...
describe('computeMonthly - 所定外労働手当（割増なし）(Phase N で追加)', () => {
  ...
  test('週40h超による法定外残業：基本給+追加所定でカバー済みなので割増のみ支給', () => {
    // 6日 × 7h = 42h（週40h超で 2h 法定外残業）の週を含むケース
    // 22日勤務（4週分相当）→ 法定残業 4h ぐらい
    // 旧バグ: 22日×7h=154h を基本給+追加所定で 1.0倍 + 法定外残業4h × 1.25 = 二重支給
    ...
    // 法定外残業 (週40h超) は発生する
    expect(w.legalOtHours).toBeGreaterThan(0)
    // 修正後: 法定外残業手当は 0.25倍 のみ（基本1.0倍は基本給+追加所定で支払い済み）
    const expectedOt = Math.round(1500 * 0.25 * (w.legalOtHours || 0))
    expect(w.otAllowance).toBe(expectedOt)
    // 所定外労働は0 (totalDailyExcess = 0、各日7h丁度)
