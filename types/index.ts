export interface Worker {
   ...
   * 短縮した分だけ実労働が延びるため、その時間を毎日の所定外労働として支払う。
   * 法定内（1日8時間以内）だが、雇用契約書の「所定超 25%」に合わせて **残業と同じ単価（時給×1.25）** で計算する（2026-09-15 代表決定）。
   * 出勤した日にだけ加算し、有給・休み・補償日・帰国中の日は対象外。
   */
  breakShortenMin?: number
  /** 上記の適用開始月 'YYYYMM'。これより前の月は一切影響しない */
  breakShortenFrom?: string
  /**
   * 社有車を運転する可能性があるか（運転者の選択肢に出すか）。
   * 未設定なら「日本人=あり／外国人=なし」を既定とする（免許は国籍と別軸なので、
   * 例外は人員マスタのチェックで上書きできる）。
   */
  canDrive?: boolean
  /**
   * 禁煙手当（年額3万円・賞与に上乗せ）の対象か。煙草を吸わない社員に支給する。
   * 金額は lib/jp-wage.ts の NON_SMOKER_ALLOWANCE。
   */
  nonSmoker?: boolean
  /**
   * 扶養している子の誕生年月 'YYYY-MM' の配列（子ども手当の計算用）。
   * 18歳の誕生日を迎える年まで対象。年長順に第1子=3万/第2子=5万/第3子以降=7万。
   * 奥寺さんの管理簿が「誕生月」単位のため、日は持たない。
   */
  children?: string[]
  /**
...
export interface Site {
   ...
   * - support: 応援（他社の現場に応援として入る。例: 銀座・姉ヶ崎の類）
