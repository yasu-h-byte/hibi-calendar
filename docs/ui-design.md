...
| グループ名 | **BOWHEAD HOLDINGS** | サイドバーで DEDURA＋ の下に小さく添える |
| 会社名 | **HIBI CONSTRUCTION** | ログイン画面・公開カレンダー・評価帳票などの印字・LINE通知文の差出人 |
...
常設表示は個社名ではなくグループ名（`DEDURA_GROUP`）を出す。会社名は上表の場所に残っている。
