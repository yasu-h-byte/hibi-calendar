...
| 雇用区分 | 適用方式 | 根拠 |
|---|---|---|
| 日本人 日給制 | 通常賃金（日額 × 1日） | 日次の日額を採用 |
| 日本人 月給制 | 通常賃金（月給は減額せず満額支給） | 月給制では当日も基本給範囲内 |
| 外国人 時給制（新ルール） | 通常賃金（時給 × 7h = ベース日数1日分） | 基本給は 時給×20日×7h 固定なので減額なし |
| 外国人 月給制（新ルール） | 通常賃金（月給は減額せず満額支給） | 同上 |

**実装上の挙動:**
- 出面入力で `p:1` を入力した日は、各雇用区分の「通常賃金」が自動的に支払われる
- 月給制は基本給が固定なので「有給で減額されない」自動効果
- 時給制も `基本給(固定) = 時給 × 20日 × 7h` のため、有給日もこの枠内（欠勤控除されない）
