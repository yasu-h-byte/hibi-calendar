...
有給管理画面の「月別」タブでスタッフごとの各月の取得日数を表示。
...
- **Phase 4（完了）**: 繰越ボタンの整理、スマホP入力禁止の確認（既に実装済みだった）
- **Phase 5**: 年5日取得義務の時季指定UI（TODO）
- **Phase 6**: 退職時清算・期末買取記録UI（TODO）
- **Phase 7**: 有給管理簿Excel出力（TODO）
